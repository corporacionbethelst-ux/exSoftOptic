...
@router.post("/from-laboratorio/{orden_id}", response_model=GarantiaResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permissions(["garantias.crear"]))])
...
@router.get("/{garantia_id}", response_model=GarantiaResponse, dependencies=[Depends(require_permissions(["garantias.leer"]))])
...
@router.post("/{garantia_id}/reclamaciones", response_model=ReclamacionGarantiaResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permissions(["garantias.reclamaciones.crear"]))])
...
@router.post("/reclamaciones/{reclamacion_id}/resolver", response_model=GarantiaResponse, dependencies=[Depends(require_permissions(["garantias.reclamaciones.resolver"]))])
