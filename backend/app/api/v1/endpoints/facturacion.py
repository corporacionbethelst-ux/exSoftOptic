...
async def emitir_factura(
    ...
    try:
        ...
        if idempotency:
            response_body = jsonable_encoder(FacturaResponse.model_validate(factura))
...
async def cancelar_factura(
    factura_id: UUID,
    payload: FacturaCancelarRequest,
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_active_user),
    idempotency_key: str | None = Header(None, alias="Idempotency-Key"),
):
    ...
    if idempotency_key:
        try:
            idempotency = await IdempotencyService(db).start(
                empresa_id=current_user.empresa_id,
                scope="facturacion.cancelar",
                key=idempotency_key,
                request_payload={"factura_id": str(factura_id), **payload.model_dump()},
            )
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
        if idempotency.replay:
