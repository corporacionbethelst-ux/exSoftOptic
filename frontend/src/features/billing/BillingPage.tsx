...
export function BillingPage() {
  ...
  return <section className="page-stack">
    ...
    <SectionPanel title="Detalle de factura" footer={<span className="muted compact">{selectedInvoice ? selectedInvoice.folio : 'Selecciona una factura'}</span>}>
      <div className="module-grid two-columns"><div className="alert-list"><div className="alert-item"><strong>Estado fiscal</strong><StatusBadge tone={tone(selectedInvoice?.estado ?? '')}>{selectedInvoice?.estado ?? '—'}</StatusBadge><span>Proveedor {selectedInvoice?.proveedor ?? '—'} · Moneda {selectedInvoice?.moneda ?? '—'}</span></div><div className="alert-item"><strong>Totales</strong><span>Subtotal {money(selectedInvoice?.subtotal)} · Impuestos {money(selectedInvoice?.impuestos)} · Total {money(selectedInvoice?.total)}</span></div><div className="alert-item"><strong>UUID fiscal</strong><span className="compact-id">{selectedInvoice?.uuid_fiscal ?? 'Pendiente'}</span>{selectedInvoice?.fecha_timbrado ? <small>{new Date(selectedInvoice.fecha_timbrado).toLocaleString()}</small> : null}</div><div className="toolbar compact-toolbar">{selectedInvoice?.xml_url ? <a className="button-link" href={selectedInvoice.xml_url} target="_blank" rel="noreferrer">XML</a> : null}{selectedInvoice?.pdf_url ? <a className="button-link" href={selectedInvoice.pdf_url} target="_blank" rel="noreferrer">PDF</a> : null}</div></div><div className="table-wrap compact-table"><table><thead><tr><th>Producto</th><th>Descripción</th><th>Cantidad</th><th>Importe</th></tr></thead><tbody>{selectedInvoice?.lineas?.map((line) => <tr key={line.id}><td className="compact-id">{line.producto_id}</td><td>{line.descripcion}</td><td>{line.cantidad}</td><td>{money(line.importe)}</td></tr>)}</tbody></table></div></div><div className="alert-list reminder-form">{selectedInvoice?.eventos?.map((event) => <div className="alert-item" key={event.id}><strong>{event.tipo_evento}</strong><span>{event.descripcion}</span><small>{new Date(event.fecha).toLocaleString()}</small></div>)}</div>
    </SectionPanel>
    ...
    <SectionPanel title="Detalle de garantía" footer={<span className="muted compact">{selectedWarranty ? selectedWarranty.folio : 'Selecciona una garantía'}</span>}>
      <div className="module-grid two-columns"><div className="alert-list"><div className="alert-item"><strong>Vigencia</strong><span>{selectedWarranty?.fecha_inicio ?? '—'} - {selectedWarranty?.fecha_fin ?? '—'}</span><StatusBadge tone={tone(selectedWarranty?.estado ?? '')}>{selectedWarranty?.estado ?? '—'}</StatusBadge></div><div className="alert-item"><strong>Descripción</strong><span>{selectedWarranty?.descripcion ?? 'Sin descripción'}</span></div><div className="alert-item"><strong>Condiciones</strong><span>{selectedWarranty?.condiciones ?? 'Sin condiciones'}</span></div></div><div className="alert-list">{selectedWarranty?.eventos?.map((event) => <div className="alert-item" key={event.id}><strong>{event.tipo_evento}</strong><span>{event.descripcion}</span><small>{new Date(event.fecha).toLocaleString()}</small></div>)}</div></div>
    </SectionPanel>
